...
export function AreaCard({ area, variant = "image" }: { area: Area; variant?: "image" | "solid" }) {
  const heightCls = "h-56";
  ...
  if (variant === "solid") {
    return (
      <Link
        ...
        className={cn(
          "group flex flex-col justify-between rounded-2xl bg-primary p-6 text-white shadow-card transition hover:brightness-95",
          heightCls,
        )}
      >
  ...
  return (
    <Link
      href={`/areas/${area.slug}`}
      className={cn(
        "group relative block overflow-hidden rounded-2xl shadow-card transition hover:-translate-y-1",
        heightCls,
      )}
    >
      <Image
      ...
      <div className="absolute inset-x-0 bottom-0 p-5 text-white">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {area.highlights.slice(0, 2).map((h) => (
            <span
              key={h}
              className="rounded-full bg-primary px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider"
            >
              {h}
            </span>
          ))}
